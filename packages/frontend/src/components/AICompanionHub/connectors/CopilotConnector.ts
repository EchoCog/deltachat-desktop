  ...
  async authenticate(): Promise<boolean> {
    try {
      ...
      if (!testResponse.ok) {
        ...
        throw new Error(`Copilot authentication failed: ${errorData.message || testResponse.statusText}`)
      }
...
- Ensure security best practices are followed`
  ...
  private isCodeRelatedQuery(query: string): boolean {
    const codeRelatedKeywords = [
      ...
      'bug', 'error', 'debug', 'fix', 'feature',
      'javascript', 'typescript', 'python', 'java', 'c#', 'c++',
      'html', 'css', 'api', 'endpoint', 'database',
      'algorithm', 'syntax', 'compile', 'runtime'
    ]
    
    const lowerQuery = query.toLowerCase()
    return codeRelatedKeywords.some(keyword => lowerQuery.includes(keyword))
      || lowerQuery.includes('```')
      || /\b(if|for|while|switch|try|catch|function|class|import|export|const|let|var)\b/.test(lowerQuery)
  }
  ...
  async generateResponse(context: ConversationContext): Promise<AIResponse> {
    try {
      ...
      const apiEndpoint = this.copilotConfig.apiEndpoint || 'https://api.github.com/copilot/completions'
  ...
  async getCodeSuggestions(
    ...
    try {
      ...
      const apiEndpoint = `${this.copilotConfig.apiEndpoint || 'https://api.github.com/copilot'}/suggestions`
