...
import { BaseConnector, AIConnectorConfig, AICapability, ConversationContext, FunctionDefinition, AIResponse } from './BaseConnector'

// Claude-specific configuration options
export interface ClaudeConfig extends AIConnectorConfig {
  // Anthropic-specific parameters
  apiVersion?: string
  modelName: 'claude-3-opus-20240229' | 'claude-3-sonnet-20240229' | 'claude-3-haiku-20240307' | 'claude-2.1' | 'claude-2.0' | 'claude-instant-1.2'
  maxTokens?: number
  topP?: number
...
export class ClaudeConnector extends BaseConnector {
    ...
    try {
      ...
      const testResponse = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        headers: {
          ...
          'anthropic-version': this.claudeConfig.anthropicVersion || 'bedrock-2023-05-31'
        },
        body: JSON.stringify({
      ...
      if (!testResponse.ok) {
        ...
        throw new Error(`Claude API authentication failed: ${errorData.error?.message || testResponse.statusText}`)
      }
   ...
   */
  private formatClaudeMessages(context: ConversationContext): ClaudeRequestMessage[] {
  ...
  async generateResponse(
    ...
    try {
      ...
      if (this.claudeConfig.systemPrompt && !messages.some(m => m.role === 'system')) {
