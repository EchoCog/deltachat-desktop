...
import { BaseConnector, AIConnectorConfig, AICapability, ConversationContext, AIResponse } from './BaseConnector'
...
export class CharacterAIConnector extends BaseConnector {
  ...
  async authenticate(): Promise<boolean> {
    try {
      ...
      const authResponse = await fetch('https://beta.character.ai/chat/auth/login/', {
        method: 'POST',
        headers: {
      ...
      if (!authResponse.ok) {
        ...
        throw new Error(`Character.AI authentication failed: ${errorData.error || authResponse.statusText}`)
      }
  ...
  private async loadCharacterDefinition(characterId: string): Promise<any> {
    try {
      ...
      if (!this.authToken) {
        throw new Error('Authentication required before loading character definitions')
      }
      
      const response = await fetch(`https://beta.character.ai/chat/character/${characterId}/`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to load character definition: ${errorData.error || response.statusText}`)
      }
      ...
      console.error(`Error loading character definition for ${characterId}:`, error)
      throw error
    }
  }
  ...
  async getAvailableCharacters(): Promise<any[]> {
    try {
      ...
      const response = await fetch('https://beta.character.ai/chat/characters/', {
        method: 'GET',
        headers: {
          'Authorization': `Token ${this.authToken}`
        }
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(`Failed to load available characters: ${errorData.error || response.statusText}`)
      }
   ...
   */
  async startNewChat(characterId: string, initialMessage?: string): Promise<string> {
    try {
      if (!this.authToken) {
        await this.authenticate()
      }
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(`Failed to start new chat: ${errorData.error || response.statusText}`)
      }
  ...
  async generateResponse(context: ConversationContext): Promise<AIResponse> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(`Character.AI API error: ${errorData.error || response.statusText}`)
      }
      ...
      return {
        messageId: `char_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`,
  ...
  async searchCharacters(query: string): Promise<any[]> {
    try {
      ...
      const response = await fetch(`https://beta.character.ai/chat/characters/search/?query=${encodeURIComponent(query)}`, {
        method: 'GET',
        headers: {
          'Authorization': `Token ${this.authToken}`
        }
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(`Failed to search characters: ${errorData.error || response.statusText}`)
      }
  ...
  async createCharacter(definition: {
    ...
    try {
      ...
      const response = await fetch('https://beta.character.ai/chat/characters/create/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Token ${this.authToken}`
