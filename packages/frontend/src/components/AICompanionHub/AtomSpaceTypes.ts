...
export interface HyperEdge extends Link {
  type: AtomType.HYPEREDGE | AtomType.RECURSIVE_LINK | AtomType.EMERGENT_PATTERN | AtomType.CONSCIOUSNESS_LINK;
...
export interface DeepTreeEchoState {
  ...
  reflectionStack: {                 // Stack of recursive reflections
    level: number;
    focusAtoms: string[];
    insights: string[];
  }[];
  emergentPatterns: Record<string, {
    patternId: string;
    strength: number;
...
export const createAtomSpace = (): AtomSpace => ({
  ...
  indices: {
    byType: Object.values(AtomType).reduce((acc, type) => {
