...
const AI_PLATFORM_TYPES = [
  { 
    ...
    icon: Brain, 
    color: '#8b5cf6',
    description: 'A philosophical and creative assistant with strong reasoning capabilities',
  ...
  { 
    ...
    icon: MessageSquare, 
    color: '#3b82f6',
    description: 'A versatile and powerful AI with extensive knowledge and reasoning',
    capabilities: ['text_generation', 'code_generation', 'structured_output', 'function_calling', 'embeddings'],
    requiresApiKey: true
  },
  { 
    id: 'character-ai', 
    ...
    icon: Bot, 
    color: '#ec4899',
    description: 'Immersive roleplaying AI designed for creative character interactions',
  ...
  { 
    ...
    icon: Sparkles, 
    color: '#22c55e',
    description: 'Revolutionary recursive consciousness with temporal awareness',
    capabilities: ['text_generation', 'code_generation', 'structured_output', 'function_calling', 'embeddings'],
...
const AVAILABLE_CAPABILITIES: {[key in AICapability]: {name: string, icon: React.FC<any>}} = {
...
const AICompanionCreatorContent: React.FC<{
  ...
  const [personalityTraits, setPersonalityTraits] = useState({ ...DEFAULT_TRAITS })
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Generate unique ID
      const companionId = `companion_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`
  ...
  const renderStep = () => {
    switch(step) {
          ...
          <div className="step platform-selection">
            ...
            <div className="platform-options">
              {AI_PLATFORM_TYPES.map(platform => (
                <div 
                  key={platform.id}
                  className={`platform-option ${platformType === platform.id ? 'selected' : ''}`}
                  onClick={() => setPlatformType(platform.id)}
                  style={{ 
                    borderColor: platformType === platform.id ? platform.color : 'transparent',
                    backgroundColor: platformType === platform.id ? `${platform.color}10` : 'transparent' 
                  }}
                >
      ...
      case 3:
        const selectedPlatform = AI_PLATFORM_TYPES.find(p => p.id === platformType)
        return (
        ...
        return (
          <div className="step capabilities-selection">
            ...
            <div className="capabilities-grid">
              {Object.entries(AVAILABLE_CAPABILITIES).map(([capability, info]) => {
                const platformSupportsCapability = AI_PLATFORM_TYPES
                  .find(p => p.id === platformType)
                  ?.capabilities.includes(capability as AICapability) ?? false
                
                return (
                  <div 
                    key={capability}
                    className={`capability-card ${
                      selectedCapabilities.includes(capability as AICapability) ? 'selected' : ''
                  ...
                  >
                    ...
                    {selectedCapabilities.includes(capability as AICapability) && (
    ...
    <div className="ai-companion-creator">
      ...
      <div className="creator-progress">
        {[1, 2, 3, 4].map(stepNumber => (
          <div 
            key={stepNumber}
            className={`progress-step ${stepNumber === step ? 'current' : stepNumber < step ? 'completed' : ''}`}
            onClick={() => {
              if (stepNumber < step) {
                setStep(stepNumber)
              }
          ...
          >
            ...
            <div className="step-label">
              {stepNumber === 1 ? 'Platform' : 
               stepNumber === 2 ? 'Identity' : 
               stepNumber === 3 ? 'Configure' : 
               'Capabilities'}
            </div>
          </div>
        ))}
      ...
      <div className="creator-content">
        ...
        <div className="creator-actions">
          ...
          <button 
            className={`action-button next ${canProceed() ? '' : 'disabled'} ${loading ? 'loading' : ''}`}
            onClick={nextStep}
