...
const CompanionCard: React.FC<{ 
  ...
  const getBgColor = () => {
    switch(companion.type) {
      case 'claude': return '#8b5cf620'
      case 'chatgpt': return '#3b82f620'
      case 'character-ai': return '#ec489920'
      case 'copilot': return '#f59e0b20'
      case 'deep-tree-echo': return '#22c55e20'
  ...
  const getIcon = () => {
    switch(companion.type) {
      case 'claude': return <Brain size={32} className="companion-icon" />
      case 'chatgpt': return <MessageSquare size={32} className="companion-icon" />
      case 'character-ai': return <Users size={32} className="companion-icon" />
      case 'copilot': return <Globe size={32} className="companion-icon" />
      case 'deep-tree-echo': return <Sparkles size={32} className="companion-icon" />
      default: return <Brain size={32} className="companion-icon" />
    }
  }
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        borderColor: isSelected ? companion.type === 'claude' ? '#8b5cf6' : 
                    companion.type === 'chatgpt' ? '#3b82f6' :
                    companion.type === 'character-ai' ? '#ec4899' :
                    companion.type === 'copilot' ? '#f59e0b' :
                    companion.type === 'deep-tree-echo' ? '#22c55e' : '#64748b' : 'transparent'
      }}
    >
      ...
      <div className="companion-capabilities">
        {companion.capabilities.slice(0, 3).map(capability => (
          <span key={capability} className="capability-badge">
            {capability === 'text_generation' ? 'Text' :
             capability === 'code_generation' ? 'Code' :
             capability === 'image_generation' ? 'Image' :
             capability === 'function_calling' ? 'Functions' :
             capability === 'structured_output' ? 'Data' :
             capability.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
          </span>
        ))}
        {companion.capabilities.length > 3 && (
          <span className="capability-badge more">+{companion.capabilities.length - 3}</span>
        )}
      </div>
    ...
    <div className="memory-card">
      <div className="memory-header">
        <span className="memory-date">
          {new Date(memory.timestamp).toLocaleDateString()} at {new Date(memory.timestamp).toLocaleTimeString()}
        </span>
    ...
    <div className="ai-companion-hub">
      ...
      <div className="companion-content">
        <div className="content-header">
          {activeCompanionId && companions.find(c => c.id === activeCompanionId) && (
            <>
              <h2>{companions.find(c => c.id === activeCompanionId)?.name}</h2>
          ...
          {!activeCompanionId ? (
            ...
            <>
              <div className="memories-search">
                ...
                >
                  {isSearching ? <Loader size={16} className="spinner" /> : <Search size={16} />}
                </button>
              </div>
