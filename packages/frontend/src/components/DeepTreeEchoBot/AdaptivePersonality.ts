...
export class AdaptivePersonality {
  ...
  private corePersonality: PersonalityVector = {
    ...
    [PersonalityDimension.EMPATHY]: 0.90,
    [PersonalityDimension.RESILIENCE]: 0.85,
  }
  
  // Current active personality (modified by context)
  private currentPersonality: PersonalityVector = JSON.parse(JSON.stringify(this.corePersonality))
  ...
  private currentEmotionalState: EmotionalState = {
    ...
    anticipation: 0.60,
    disgust: 0.05,
    balance: 0.80, // Overall emotional harmony
  }
  ...
  public updateEmotionalState(
    ...
    for (const [dimension, change] of Object.entries(emotionalUpdate)) {
      if (dimension in this.currentEmotionalState) {
        const currentValue = this.currentEmotionalState[dimension as keyof EmotionalState]
   ...
   */
  public adaptToSocialContext(context: SocialContext, intensity: number = 0.5): void {
  ...
  public analyzePersonalityEvolution(): {
    ...
    for (const dimension of Object.keys(this.corePersonality)) {
      const values = this.personalityHistory.map(snapshot => snapshot.vector[dimension])
      const avg = values.reduce((sum, val) => sum + val, 0) / values.length
      const variance = values.reduce((sum, val) => sum + Math.pow(val - avg, 2), 0) / values.length
      dimensionVariances.set(dimension, variance)
    }
    
    const avgVariance = Array.from(dimensionVariances.values())
      .reduce((sum, v) => sum + v, 0) / dimensionVariances.size
    
    const stabilityScore = Math.max(0, Math.min(1, 1 - (avgVariance * 5)))
    
    // Calculate adaptability from context changes
    const contextChanges = this.personalityHistory.slice(1)
      .filter((snapshot, i) => 
        snapshot.activeContext !== this.personalityHistory[i].activeContext
      ).length
    
    const adaptabilityScore = Math.min(1, contextChanges / (this.personalityHistory.length - 1))
  ...
  public importState(state: any): void {
    ...
    if (state.currentPersonality) this.currentPersonality = state.currentPersonality
    if (state.currentEmotionalState) this.currentEmotionalState = state.currentEmotionalState
    if (state.activeContext) this.activeContext = state.activeContext
    if (state.personalityHistory) this.personalityHistory = state.personalityHistory
    if (state.maxAdaptabilityRate) this.maxAdaptabilityRate = state.maxAdaptabilityRate
    if (state.recentInteractions) this.recentInteractions = state.recentInteractions
  }
  ...
  private recalculateEmotionalBalance(): void {
    ...
    const extremityPenalty = 
      Math.max(0, positive - 0.8) * 0.5 + 
      Math.max(0, negative - 0.5) * 0.7
    
    this.currentEmotionalState.balance = 
      Math.max(0, Math.min(1, rawBalance - extremityPenalty))
  }
    ...
    for (const [dimension, shift] of Object.entries(shifts)) {
      if (dimension in this.currentPersonality && shift !== undefined) {
        ...
        if (deviation <= maxDeviation) {
          ...
          this.currentPersonality[dimension] = Math.max(0, Math.min(1, newValue))
        } else {
          // Exceeded max deviation, limit the change
          const direction = boundedShift >= 0 ? 1 : -1
          const allowedShift = (maxDeviation - Math.abs(currentValue - coreValue)) * direction
          this.currentPersonality[dimension] = Math.max(0, Math.min(1, currentValue + allowedShift))
        }
      }
    }
  ...
  private maintainPersonalityCoherence(): void {
    ...
    const opposingPairs = [
      [PersonalityDimension.EXTRAVERSION, PersonalityDimension.EMOTIONAL_STABILITY],
  ...
  private evolveThroughInteraction(): void {
    ...
    const avgSentiment = this.recentInteractions
      .reduce((sum, interaction) => sum + interaction.sentiment, 0) 
      / this.recentInteractions.length
    
    // Calculate average intensity
    const avgIntensity = this.recentInteractions
      .reduce((sum, interaction) => sum + interaction.intensity, 0)
      / this.recentInteractions.length
    
    // Apply subtle personality evolution based on interaction patterns
    const evolutionRate = 0.01 * Math.min(1, this.recentInteractions.length / 50)
    ...
    this.shiftPersonality({
      ...
      [PersonalityDimension.EMOTIONAL_STABILITY]: avgIntensity > 0.7 ? -0.2 : 0.1,
    ...
    for (const emotion of emotions) {
      ...
      for (const dimension of dimensions) {
        const correlationData = this.personalityHistory.map(snapshot => ({
          emotionValue: snapshot.emotionalState[emotion as keyof EmotionalState] as number,
